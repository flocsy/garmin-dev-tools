...
enum HintType {HINT_NONE = 0, HINT_SINGLE_CLICK = 1, HINT_DOUBLE_CLICK = 2, HINT_LONG_PRESS = 3, HINT_MENU = 4}
...
function draw_key_hint_enter(dc as Graphics.Dc) as Void {
    draw_key_hint(dc, KEY_ENTER_ARC_DEGREE, KEY_ENTER_BAR_X, KEY_ENTER_BAR_Y, KEY_ENTER_BAR_WIDTH, KEY_ENTER_BAR_HEIGHT, HINT_SINGLE_CLICK);
}
...
function draw_key_hint_esc(dc as Graphics.Dc) as Void {
    draw_key_hint(dc, KEY_ESC_ARC_DEGREE, KEY_ESC_BAR_X, KEY_ESC_BAR_Y, KEY_ESC_BAR_WIDTH, KEY_ESC_BAR_HEIGHT, HINT_SINGLE_CLICK);
}
...
function draw_key_hint_menu(dc as Graphics.Dc) as Void {
    draw_key_hint(dc, KEY_MENU_ARC_DEGREE, KEY_MENU_BAR_X, KEY_MENU_BAR_Y, KEY_MENU_BAR_WIDTH, KEY_MENU_BAR_HEIGHT, KEY_MENU_IS_HOLD ? HINT_MENU : HINT_SINGLE_CLICK);
}
...
function draw_key_hint_up(dc as Graphics.Dc) as Void {
    draw_key_hint(dc, KEY_UP_ARC_DEGREE, KEY_UP_BAR_X, KEY_UP_BAR_Y, KEY_UP_BAR_WIDTH, KEY_UP_BAR_HEIGHT, HINT_SINGLE_CLICK);
}
...
function draw_key_hint_down(dc as Graphics.Dc) as Void {
    draw_key_hint(dc, KEY_DOWN_ARC_DEGREE, KEY_DOWN_BAR_X, KEY_DOWN_BAR_Y, KEY_DOWN_BAR_WIDTH, KEY_DOWN_BAR_HEIGHT, HINT_SINGLE_CLICK);
}
...
function draw_key_hint_lap(dc as Graphics.Dc) as Void {
    draw_key_hint(dc, KEY_LAP_ARC_DEGREE, KEY_LAP_BAR_X, KEY_LAP_BAR_Y, KEY_LAP_BAR_WIDTH, KEY_LAP_BAR_HEIGHT, HINT_SINGLE_CLICK);
}
...
function draw_key_hint_start(dc as Graphics.Dc) as Void {
    draw_key_hint(dc, KEY_START_ARC_DEGREE, KEY_START_BAR_X, KEY_START_BAR_Y, KEY_START_BAR_WIDTH, KEY_START_BAR_HEIGHT, HINT_SINGLE_CLICK);
}
...
function draw_key_hint_clock(dc as Graphics.Dc) as Void {
    draw_key_hint(dc, KEY_CLOCK_ARC_DEGREE, KEY_CLOCK_BAR_X, KEY_CLOCK_BAR_Y, KEY_CLOCK_BAR_WIDTH, KEY_CLOCK_BAR_HEIGHT, KEY_CLOCK_IS_HOLD ? HINT_LONG_PRESS : HINT_SINGLE_CLICK);
}
...
function draw_key_hint_onselect(dc as Graphics.Dc) as Void {
    draw_key_hint(dc, KEY_ONSELECT_ARC_DEGREE, KEY_ONSELECT_BAR_X, KEY_ONSELECT_BAR_Y, KEY_ONSELECT_BAR_WIDTH, KEY_ONSELECT_BAR_HEIGHT, HINT_SINGLE_CLICK);
}
...
function draw_key_hint_onback(dc as Graphics.Dc) as Void {
    draw_key_hint(dc, KEY_ONBACK_ARC_DEGREE, KEY_ONBACK_BAR_X, KEY_ONBACK_BAR_Y, KEY_ONBACK_BAR_WIDTH, KEY_ONBACK_BAR_HEIGHT, HINT_SINGLE_CLICK);
}
...
function draw_key_hint_onmenu(dc as Graphics.Dc) as Void {
    draw_key_hint(dc, KEY_ONMENU_ARC_DEGREE, KEY_ONMENU_BAR_X, KEY_ONMENU_BAR_Y, KEY_ONMENU_BAR_WIDTH, KEY_ONMENU_BAR_HEIGHT, KEY_ONMENU_IS_HOLD ? HINT_MENU : HINT_SINGLE_CLICK);
}
...
function draw_key_hint_previouspage(dc as Graphics.Dc) as Void {
    draw_key_hint(dc, KEY_PREVIOUSPAGE_ARC_DEGREE, KEY_PREVIOUSPAGE_BAR_X, KEY_PREVIOUSPAGE_BAR_Y, KEY_PREVIOUSPAGE_BAR_WIDTH, KEY_PREVIOUSPAGE_BAR_HEIGHT, HINT_SINGLE_CLICK);
}
...
function draw_key_hint_nextpage(dc as Graphics.Dc) as Void {
    draw_key_hint(dc, KEY_NEXTPAGE_ARC_DEGREE, KEY_NEXTPAGE_BAR_X, KEY_NEXTPAGE_BAR_Y, KEY_NEXTPAGE_BAR_WIDTH, KEY_NEXTPAGE_BAR_HEIGHT, HINT_SINGLE_CLICK);
}
...
function draw_key_hint(dc as Graphics.Dc, degree as Float, barX as Number, barY as Number, barW as Number, barH as Number, hintType as HintType) as Void {
    var additionalHintWidth = hintType == HINT_LONG_PRESS ? 2 : 0;
    if (DEBUG_LAYOUT) {
    // var x0 = x + width / 2;
    // var y0 = y + height / 2;
    // log("drawKeyHint: x: " + x + ", y: " + y + ", w: " + width + ", h: " + height + ", SHAPE_RECTANGLE: " + SHAPE_RECTANGLE + ", SHAPE_ROUND: " + SHAPE_ROUND);
    ...
    if (KEY_HINT_IS_ARC) {
        ...
        if ((KEY_ID_MENU || KEY_BEHAVIOR_ONMENU) && hintType == HINT_MENU) {
            dc.drawArc(halfWidth, dc.getHeight() / 2, halfWidth /*- (KEY_HINT_ARC_WIDTH - 1) / 2*/, Graphics.ARC_COUNTER_CLOCKWISE, degree - 8, degree - 6);
            dc.drawArc(halfWidth, dc.getHeight() / 2, halfWidth /*- (KEY_HINT_ARC_WIDTH - 1) / 2*/, Graphics.ARC_COUNTER_CLOCKWISE, degree - 1, degree + 1);
            dc.drawArc(halfWidth, dc.getHeight() / 2, halfWidth /*- (KEY_HINT_ARC_WIDTH - 1) / 2*/, Graphics.ARC_COUNTER_CLOCKWISE, degree + 6, degree + 8);
        } else {
            dc.drawArc(halfWidth, dc.getHeight() / 2, halfWidth /*- (KEY_HINT_ARC_WIDTH - 1) / 2*/, Graphics.ARC_COUNTER_CLOCKWISE, degree - 8, degree + 8);
        }
    }
    if (KEY_HINT_IS_BAR || (KEY_HINT_IS_ARC && barW > 0)) {
        var penW = KEY_HINT_BAR_WIDTH + additionalHintWidth;
        dc.setPenWidth(penW);
        if ((KEY_ID_MENU || KEY_BEHAVIOR_ONMENU) && hintType == HINT_MENU) {
            if (barW < barH) {
                dc.drawRectangle(barX, barY + (barH - 5 * penW) / 2, barW, barW);
                dc.drawRectangle(barX, barY + (barH - penW) / 2, barW, barW);
                dc.drawRectangle(barX, barY + (barH + 3 * penW) / 2, barW, barW);
            } else {
                dc.drawRectangle(barX + (barW - 5 * penW) / 2, barY, barH, barH);
                dc.drawRectangle(barX + (barW - penW) / 2, barY, barH, barH);
                dc.drawRectangle(barX + (barW + 3 * penW) / 2, barY, barH, barH);
            }
        } else {
            dc.drawRectangle(barX, barY, barW, barH);
        }
        // logRelease("draw_key_hint: barX: " + barX + ", barY: " + barY + ", barW:" + barW + ", barH:" + barH + ", hintType:" + hintType);
    }
}
