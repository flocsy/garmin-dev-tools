...
def label_font(dev):
                ...
                for field_idx, field in enumerate(datafield['fields']):
                    ...
                    if key in key2fonts:
                        if key2fonts[key][0] == label_font:
                            ...
                            print_warn(f"{dev}: label_font: conflicting key: {key}: {key2fonts[key]} vs [{label_font}] ({field_name})")
                    else:
    ...
    default_label_x, default_label_x_occurrences = get_most_frequent_value_and_occurrences(hash2label_x)
    default_label_y, default_label_y_occurrences = get_most_frequent_value_and_occurrences(hash2label_y)
    default_label_justification = get_most_frequent_value(hash2label_justification)

    default_data_x, default_data_x_occurrences = get_most_frequent_value_and_occurrences(hash2data_x)
    default_data_y, default_data_y_occurrences = get_most_frequent_value_and_occurrences(hash2data_y)
    ...
    with open(device_file, 'w') as output:
        ...
        for field_part in ['label', 'data']:
            ...
            default_x_occurrences = loc[f"default_{field_part}_x_occurrences"]
            output.write(f"(:datafield, :datafield_hash, :datafield_{field_part}_x) const DEFAULT_{CAPITAL}_X = {default_x}; /* x{default_x_occurrences} */\n")
            ...
            default_y_occurrences = loc[f"default_{field_part}_y_occurrences"]
            output.write(f"(:datafield, :datafield_hash, :datafield_{field_part}_y) const DEFAULT_{CAPITAL}_Y = {default_y}; /* x{default_y_occurrences} */\n")
...
def datafield_detector(dev):
    ...
    if 'display' not in device['simulator']:
        print(f"{dev}: no display shape")
        return ['', '', '']
    if device['simulator']['display']['shape'] == 'rectangle':
        print_warn(f"{dev}: datafield_detector: rectangle display");
        # return ['', '', '']
        sourceDirs = list(filter(None, [has_directory('source-features/rectangle'), has_directory(f"{SOURCE_DEVICES_DIR}/{dev}")]))
        return [';'.join(sourceDirs), '', '']
    if 'layouts' not in device['simulator']:
        print(f"{dev}: no layouts")
        return ['', '', '']
    ...
    hash2layout = {}
    display_location = device['simulator']['display']['location']
    width = display_location['width']
    height = display_location['height']
    height_bits = math.ceil(math.log2(display_location['height']))

    for layout in device['simulator']['layouts']:
        for datafield in layout['datafields']['datafields']:
            layout_key2field_name = {}
            layout_name = datafield['name']
            layout_dict = {
            ...
            try:
                for field_idx, field in enumerate(datafield['fields']):
                    ...
                    if hash not in hash2layout:
                        hash2layout[hash] = []
                    if layout_dict not in hash2layout[hash]:
                        hash2layout[hash].append(layout_dict)
                    if key in key2field_name:
                        key2field_name[key].append(field_name)
                        hash2field_name[hash].append(field_name)
                        if layout_dict['status'] < 1:
                            layout_dict['status'] = 1
                    else:
                        key2field_name[key] = [field_name]
                        hash2field_name[hash] = [field_name]
                    if key in layout_key2field_name:
                        layout_key2field_name[key].append(field_name)
                        layout_dict['status'] = 3
                    else:
                        layout_key2field_name[key] = [field_name]
                    # print(f"{dev}:{key}:{label_font} ({field_name})")
            except Exception as e:
                sys.exit(f"{COLOR_RED}Error: {dev}: {e=} in {field_name}: {field}{COLOR_RESET}")
            for idx, key in enumerate(layout_key2field_name):
                field_names = layout_key2field_name[key]
                if len(field_names) > 1:
                    # print(layout_dict['status'])
                    print_error(f"{dev}: datafield_detector: conflicting key in layout: {layout_name}: {key}: {field_names}")
    for idx, key in enumerate(key2field_name):
        field_names = key2field_name[key]
        if len(field_names) > 1 and False:
            print(f"{dev}: datafield_detector: conflicting key: {key}: {field_names}")
    hash2status = {}
    # print(f"{dev}: hash2layout:{hash2layout}")
    for hash in hash2layout:
        status = 0
        status_sum = 0
        status_counter = {0:0, 1:0, 3:0}
        for layout_dict in hash2layout[hash]:
            status_sum += layout_dict['status']
            status_counter[layout_dict['status']] += 1
            # print(f"{dev}: h:{hash},n:{layout_dict['short_name']}, s:{layout_dict['status']}")
        if status_counter[3] > 0:
            if len(hash2layout[hash]) < len(hash2field_name[hash]):
                if status_counter[3] == len(hash2layout[hash]):
                    status = 3
                else:
                    status = 2
            else:
                status = 1
        elif status_counter[1] > 0 and len(hash2layout[hash]) > 1:
            status = 1
        else:
            status = 0
        hash2status[hash] = status
        # print(f"{dev}: h:{hash}, s:{status}")

    device_dir = f"{SOURCE_DEVICES_DIR}/{dev}"
    device_file = f"{device_dir}/datafield_detector.mc"
    os.makedirs(device_dir, 0o755, True)
    with open(device_file, 'w') as output:
        # output.write(f"// GENERATED for {dev} by {GENERATROR_SIGNATURE}\n\n");
        output.write(f"// GENERATED by {GENERATROR_SIGNATURE}\n\n");
        ...
        output.write("(:datafield_detector)\n")
        output.write("const DATAFIELD_HASH_2_FIELD_NAME = {\n")
        for hash, field_names in hash2field_name.items():
            field_name_str = ''
            for field_name in field_names:
                if field_name_str:
                    field_name_str += '|'
                field_name_str += to_short_field_name(field_name)
            # print(f"{dev}: hash2layout[{hash}]:{hash2layout[hash]}")
            output.write(f" {hash} /*{hash2comment[hash]}:{hash2status[hash]}*/ => \"{field_name_str}\",\n")
        output.write("} as Dictionary<Number, String>;\n")

        output.write("\n(:datafield_detector)\n")
        output.write("const DATAFIELD_HASH_2_FIELD_STATUS = {\n")
        for hash, field_names in hash2field_name.items():
            output.write(f" {hash} /*{hash2comment[hash]}:{hash2status[hash]}*/ => {hash2status[hash]},\n")
        output.write("} as Dictionary<Number, Number>;\n")
